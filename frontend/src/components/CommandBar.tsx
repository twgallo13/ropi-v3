...
export default function CommandBar({ open, onClose }: CommandBarProps) {
  ...
  const NAV_COMMANDS: CommandItem[] = useMemo(
    () => [
      ...
      // TALLY-165 — Buyer Review renamed to Buyer Cockpit; Cadence Review,
      // MAP Conflict, MAP Removal, and Pricing Discrepancy entries kept for
      // command-bar discoverability but routed to their consolidated
      // destinations (Buyer Cockpit tabs or MAP Policy tabs).
      { type: "navigation", label: "Buyer Cockpit", sublabel: "/buyer-review", icon: "🛒", action: () => go("/buyer-review") },
      { type: "navigation", label: "Cadence Review (Buyer Cockpit)", sublabel: "/buyer-review?tab=cadence", icon: "📊", action: () => go("/buyer-review?tab=cadence") },
      ...
      { type: "navigation", label: "MAP Policy", sublabel: "/map-policy", icon: "⚠️", action: () => go("/map-policy") },
      { type: "navigation", label: "MAP Conflict (MAP Policy)", sublabel: "/map-policy?tab=conflict", icon: "⚠️", action: () => go("/map-policy?tab=conflict") },
      { type: "navigation", label: "MAP Removal (MAP Policy)", sublabel: "/map-policy?tab=removal", icon: "🗑️", action: () => go("/map-policy?tab=removal") },
      { type: "navigation", label: "Pricing Discrepancy (Buyer Cockpit)", sublabel: "/buyer-review?tab=pricing", icon: "💲", action: () => go("/buyer-review?tab=pricing") },
