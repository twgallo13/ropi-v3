/**
 * TALLY-SETTINGS-UX Phase 3 / A.3 PR4 — Search Settings admin page.
 * Identical shape to LaunchSettingsPage; mounted at /admin/infrastructure/search.
 */
...
const isRowActive = (r: SearchSettingEntry) => r.is_active === true;
function compareRows(a: SearchSettingEntry, b: SearchSettingEntry, s: { key: string; dir: "asc" | "desc" }): number {
...
export default function SearchSettingsPage() {
  const [rows, setRows] = useState<SearchSettingEntry[]>([]);
  ...
  const [deactivateTarget, setDeactivateTarget] = useState<SearchSettingEntry | null>(null);
  const [deactivateError, setDeactivateError] = useState<string | null>(null);
  const [reactivateTarget, setReactivateTarget] = useState<SearchSettingEntry | null>(null);
  const [reactivateError, setReactivateError] = useState<string | null>(null);

  async function load() {
    setLoading(true); setError(null);
    try { setRows(await fetchSearchSettings(false)); }
  ...
  const baseColumns: AdminCrudColumn<SearchSettingEntry>[] = [
  ...
  const reactivateColumn: AdminCrudColumn<SearchSettingEntry> = {
  ...
  return (
    <RoleGate>
      <div className="max-w-6xl mx-auto p-6">
        <Link to="/admin/infrastructure" className="text-sm text-blue-600 hover:underline">← Infrastructure</Link>
        <h1 className="text-2xl font-bold mt-2 mb-1">🔍 Search Settings</h1>
        <p className="text-gray-600 mb-6">Manage search runtime configuration. Changes apply on next search.</p>
        ...
        <AdminCrudTable
          ...
          isLoading={loading} emptyMessage="No search settings."
          sortState={sortState} onSortChange={setSortState}
        />
        {editorMode !== null && (
          <SearchSettingEditor mode={editorMode}
        ...
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            try {
              await deactivateSearchSetting(justKey);
        ...
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            try {
              await reactivateSearchSetting(justKey);
...
interface EditorProps {
  mode: "create" | "edit";
  initial: SearchSettingEntry | null | undefined;
  onSaved: (savedKey: string, action: "created" | "updated") => void | Promise<void>;
  onCancel: () => void;
}

function SearchSettingEditor({ mode, initial, onSaved, onCancel }: EditorProps) {
  ...
  const [valueText, setValueText] = useState<string>(initial != null ? String(initial.value) : "");
  ...
  async function handleSave() {
    ...
    if (valueType === "number") {
      ...
      coercedValue = v === "true";
    } else { coercedValue = valueText; }
    setIsSaving(true);
    try {
      const payload: Partial<SearchSettingEntry> = {
      ...
      if (mode === "create") {
        const created = await createSearchSetting({ setting_key: settingKey.trim(), ...payload });
        await onSaved(created.setting_key, "created");
      } else {
        const updated = await updateSearchSetting(initial!.setting_key, payload);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
        <h2 className="text-lg font-semibold mb-4">
          {mode === "create" ? "New Search Setting" : `Edit: ${initial?.setting_key ?? ""}`}
        </h2>
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            <input id="ss-is-active" type="checkbox" checked={isActive}
              onChange={(e) => setIsActive(e.target.checked)} />
            <label htmlFor="ss-is-active" className="text-sm">is_active</label>
          </div>
