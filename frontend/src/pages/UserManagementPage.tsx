/**
 * PHASE-3.7 sub-PR 1.8 — User Management page (extracted from AdminSettingsPage).
 *
 * Mounted at /admin/governance/users under the Access & Governance pillar.
 * Lifted verbatim from AdminSettingsPage.tsx:
 *   - UsersTab (L79–L353)
 *   - AddUserModal (L354+)
 *   - EditUserModal (L537+)
 *   - inputClass (L813), Field (~L815), ModalShell (L831)
 *
 * Adds (per dispatch §1.8):
 *   - Page shell with back-link to /admin/governance + h1 + description
 *   - Inline RoleGate redirect (admin/owner only)
 *   - Search filter on email + display_name (lowercase contains)
 */
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      ...
      <p className="text-gray-600 mb-6">
        Create, edit, disable, and reset passwords for ROPI users.
      </p>
      <UsersTab />
    </div>
  );
...
// ─────────────────────────────────────
//  USERS TAB (lifted verbatim from AdminSettingsPage.tsx)
// ─────────────────────────────────────
function UsersTab() {
  ...
  // PHASE-3.7 sub-PR 1.8 — search filter on email + display_name
  ...
  // PHASE-3.7 sub-PR 1.8 — case-insensitive substring filter on email + display_name.
    ...
    <div>
      <div className="flex items-center justify-between mb-3 gap-3">
        ...
        <input
          ...
          className="flex-1 max-w-sm border border-gray-200 dark:border-gray-700 rounded px-2 py-1.5 text-sm bg-white dark:bg-gray-900"
        />
        <button
          onClick={() => setShowAdd(true)}
          className="bg-blue-600 text-white text-sm rounded-lg px-3 py-1.5 hover:bg-blue-700 whitespace-nowrap"
        >
          + Add User
        </button>
      </div>
...
// ─────────────────────────────────────
//  Shared bits (lifted page-local from AdminSettingsPage.tsx)
// ─────────────────────────────────────
const inputClass =
