  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (action === "correct_pricing") {
        const hasOffer =
          corrected_rics_offer !== undefined && corrected_rics_offer !== null;
        const hasScom = corrected_scom !== undefined && corrected_scom !== null;
        if (!hasOffer && !hasScom) {
          res.status(400).json({
            error:
              "At least one of corrected_rics_offer or corrected_scom is required",
          });
          return;
        }
        ...
        if (hasOffer) updates.rics_offer = newRicsOffer;
        if (hasScom) {
      ...
      if (action === "flag_for_review") {
        ...
        await productRef.set(
          { discrepancy_flagged_for_review: true, discrepancy_reviewer: reviewer_uid || null },
          { merge: true }
        );
        await db().collection("audit_log").add({
          product_mpn: mpn,
