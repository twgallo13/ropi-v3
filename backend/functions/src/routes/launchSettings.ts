/**
 * Launch Settings — TALLY-SETTINGS-UX Phase 3 / A.3 PR2.
 *
 * Admin CRUD for the `launch_settings` collection. Doc-id is `setting_key`.
 * Coexists with `admin_settings/launch_*` keys per C.2 (no backfill).
 *
 *   GET    /api/v1/admin/launch-settings
 *   GET    /api/v1/admin/launch-settings/:key
 *   POST   /api/v1/admin/launch-settings
 *   PUT    /api/v1/admin/launch-settings/:key
 *   DELETE /api/v1/admin/launch-settings/:key   (soft deactivation)
 *
...
type LaunchValueType = (typeof ALLOWED_VALUE_TYPES)[number];

function validateValueAgainstType(value: any, valueType: LaunchValueType): string | null {
...
async function writeLaunchSettingAudit(
  action: string,
  entityId: string,
  actorUid: string,
...
router.get(
  ...
  async (_req: AuthenticatedRequest, res: Response) => {
    try {
      const snap = await db().collection("launch_settings").get();
      const settings = snap.docs.map((d) => d.data());
      res.json({ settings });
    } catch (err: any) {
      console.error("GET /admin/launch-settings error:", err);
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const snap = await db().collection("launch_settings").doc(pathKey).get();
      if (!snap.exists) {
      ...
      console.error("GET /admin/launch-settings/:key error:", err);
      res.status(500).json({ error: err.message });
    }
  }
);
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const ref = db().collection("launch_settings").doc(key);
      ...
      await writeLaunchSettingAudit("launch_setting_created", key, req.user!.uid, {
        before: null,
        after: payload,
      });
      ...
      console.error("POST /admin/launch-settings error:", err);
      res.status(500).json({ error: err.message });
    }
  }
);
...
router.put(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const ref = db().collection("launch_settings").doc(pathKey);
      const existing = await ref.get();
      if (!existing.exists) {
        res.status(404).json({ error: `setting "${pathKey}" not found` });
        return;
      ...
      // Determine effective value_type for value validation
      const effectiveValueType: LaunchValueType =
      ...
      const refetched = (await ref.get()).data();
      await writeLaunchSettingAudit("launch_setting_updated", pathKey, req.user!.uid, {
        before: beforeData,
        after: refetched,
      ...
      console.error("PUT /admin/launch-settings/:key error:", err);
      res.status(500).json({ error: err.message });
    }
  }
);
...
router.delete(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const refetched = (await ref.get()).data();
      await writeLaunchSettingAudit("launch_setting_deleted", pathKey, req.user!.uid, {
        before: beforeData,
        after: refetched,
      });
      ...
      console.error("DELETE /admin/launch-settings/:key error:", err);
      res.status(500).json({ error: err.message });
    }
  }
);
