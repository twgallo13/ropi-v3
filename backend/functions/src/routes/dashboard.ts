...
const ROLE_KPIS: Record<string, Set<string>> = {
  product_ops: new Set([
    "incomplete_count",
...
router.get("/", requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const visible = ROLE_KPIS[role] || new Set<string>();
    ...
    if (visible.has("incomplete_count")) {
      response.kpis.incomplete_count = await countWhere(
        "products",
        "completion_state",
        "incomplete"
      );
    ...
    if (visible.has("map_conflict_count")) {
      response.kpis.map_conflict_count = await countWhere(
        "products",
        "map_conflict_active",
        true
      );
    ...
    if (visible.has("high_priority_launches")) {
      try {
        ...
        for (const l of launchSnap.docs) {
          const ld = l.data();
          const products: string[] = ld.product_mpns || [];
            ...
            if (p.completion_state !== "complete") {
              alerts.push({
                ...
                days_remaining: Math.ceil(
                  (ld.launch_date.toDate().getTime() - Date.now()) /
                    (24 * 60 * 60 * 1000)
                ),
              });
            }
