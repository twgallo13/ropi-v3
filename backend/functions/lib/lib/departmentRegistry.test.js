 ...
 */
const departmentRegistry_1 = require("../routes/departmentRegistry");
let passed = 0;
let failed = 0;
...
function entry(key, display_name, aliases, priority, is_active = true) {
    return {
        key,
        display_name,
        aliases,
...
console.log("TEST 1 — shape + sort over 4 seeded docs");
{
    ...
    const shaped = docs.map((d) => (0, departmentRegistry_1.shapeDepartmentEntry)(d.data, d.id));
    const sorted = (0, departmentRegistry_1.filterDepartmentEntries)(shaped, false).sort(departmentRegistry_1.compareDepartmentEntries);
...
console.log("TEST 2 — activeOnly filter excludes inactive");
{
    ...
    const all = (0, departmentRegistry_1.filterDepartmentEntries)(mixed, false).sort(departmentRegistry_1.compareDepartmentEntries);
    const active = (0, departmentRegistry_1.filterDepartmentEntries)(mixed, true).sort(departmentRegistry_1.compareDepartmentEntries);
...
console.log("TEST 3 — single-doc shaping for GET /:key");
{
    ...
    const e = (0, departmentRegistry_1.shapeDepartmentEntry)(data, "footwear");
...
console.log("TEST 4 — validation accepts all 4 seeded values");
{
    ...
    assert("Footwear accepted", (0, departmentRegistry_1.isDepartmentValueAllowed)("Footwear", SEEDED), true);
    assert("Clothing accepted", (0, departmentRegistry_1.isDepartmentValueAllowed)("Clothing", SEEDED), true);
    assert("Accessories accepted", (0, departmentRegistry_1.isDepartmentValueAllowed)("Accessories", SEEDED), true);
    assert("Home & Tech accepted", (0, departmentRegistry_1.isDepartmentValueAllowed)("Home & Tech", SEEDED), true);
    // Keys.
    assert("'footwear' key accepted", (0, departmentRegistry_1.isDepartmentValueAllowed)("footwear", SEEDED), true);
    assert("'home_and_tech' key accepted", (0, departmentRegistry_1.isDepartmentValueAllowed)("home_and_tech", SEEDED), true);
    // Aliases (case-insensitive).
    assert("'Shoes' alias accepted", (0, departmentRegistry_1.isDepartmentValueAllowed)("Shoes", SEEDED), true);
    assert("'apparel' (case) accepted", (0, departmentRegistry_1.isDepartmentValueAllowed)("apparel", SEEDED), true);
    assert("'HOME AND TECH' alias accepted", (0, departmentRegistry_1.isDepartmentValueAllowed)("HOME AND TECH", SEEDED), true);
}
// ─────────────────────────────────────────────────────────────────────────
...
console.log("TEST 5 — validation rejects deactivated entry (Ruling G)");
{
    ...
    assert("'Clothing' (deactivated display_name) REJECTED", (0, departmentRegistry_1.isDepartmentValueAllowed)("Clothing", withDeactivated), false);
    assert("'clothing' (deactivated key) REJECTED", (0, departmentRegistry_1.isDepartmentValueAllowed)("clothing", withDeactivated), false);
    assert("'Apparel' (deactivated alias) REJECTED", (0, departmentRegistry_1.isDepartmentValueAllowed)("Apparel", withDeactivated), false);
    // Sibling actives still accepted.
    assert("'Footwear' still accepted", (0, departmentRegistry_1.isDepartmentValueAllowed)("Footwear", withDeactivated), true);
}
// ─────────────────────────────────────────────────────────────────────────
...
{
    assert("'Underwear' rejected", (0, departmentRegistry_1.isDepartmentValueAllowed)("Underwear", SEEDED), false);
    assert("'Toys' rejected", (0, departmentRegistry_1.isDepartmentValueAllowed)("Toys", SEEDED), false);
    assert("empty string rejected", (0, departmentRegistry_1.isDepartmentValueAllowed)("", SEEDED), false);
    assert("null rejected", (0, departmentRegistry_1.isDepartmentValueAllowed)(null, SEEDED), false);
    assert("undefined rejected", (0, departmentRegistry_1.isDepartmentValueAllowed)(undefined, SEEDED), false);
    assert("whitespace-only rejected", (0, departmentRegistry_1.isDepartmentValueAllowed)("   ", SEEDED), false);
}
// ─────────────────────────────────────────────────────────────────────────
...
console.log("TEST 7 — validation is per-write, not retroactive");
{
    ...
    const allowed = (0, departmentRegistry_1.resolveAllowedDepartmentValues)(reg);
    ...
    assert("module exposes no batch-revalidate function", typeof require("../routes/departmentRegistry").revalidateAllProducts, "undefined");
}
// ─────────────────────────────────────────────────────────────────────────
...
console.log("TEST 8 — enum_source contract: registry takes precedence over options");
{
    ...
    const allowed = (0, departmentRegistry_1.resolveAllowedDepartmentValues)(SEEDED);
...
if (failed > 0)
    process.exit(1);
//# sourceMappingURL=departmentRegistry.test.js.map