...
exports.writeWeeklySnapshots = writeWeeklySnapshots;
exports.computeNeglectedInventory = computeNeglectedInventory;
exports.buildExecutiveHealth = buildExecutiveHealth;
/**
...
async function getAdminNumber(key, fallback) {
    try {
...
async function writeWeeklySnapshots() {
    ...
    await new Promise((resolve, reject) => {
        ...
        stream.on("data", (doc) => {
            ...
            const gm = typeof p.web_gm_pct === "number"
                ? p.web_gm_pct
 ...
 */
async function computeNeglectedInventory() {
    const ageThresholdDays = await getAdminNumber("neglected_age_threshold_days", 60);
    const attentionThresholdDays = await getAdminNumber("neglected_attention_threshold_days", 14);
    const now = Date.now();
    ...
    await new Promise((resolve, reject) => {
        ...
        stream.on("data", (doc) => {
            const p = doc.data();
            const firstReceivedMs = p.first_received_at?.toMillis?.() ??
            ...
            if (lastTouchMs > attentionCutoff.getTime())
                return;
            ...
            neglected.push({
                ...
                inventory_total: (Number(p.inventory_store) || 0) + (Number(p.inventory_warehouse) || 0),
...
//# sourceMappingURL=executiveProjections.js.map