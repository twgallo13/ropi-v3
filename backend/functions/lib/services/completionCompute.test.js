 ...
 */
const completionCompute_1 = require("./completionCompute");
let passed = 0;
let failed = 0;
...
function av(id, value, verification_state, origin_type) {
...
// 1. 0 blockers (fully complete) → 100% / 0 / 0 / ""
{
    ...
    const inner = (0, completionCompute_1.computeCompletionProgressPure)(REQUIRED, avs);
    const hint = (0, completionCompute_1.buildNextActionHintPure)(inner, REQUIRED);
...
//    Verifies the pure function never reads from any external state.
{
    ...
    const a = (0, completionCompute_1.computeCompletionProgressPure)(REQUIRED, avs);
    const b = (0, completionCompute_1.computeCompletionProgressPure)(REQUIRED, avs);
...
// 9. stampCompletionOnProduct writes exactly the 5 fields with merge:true.
{
    ...
    (0, completionCompute_1.stampCompletionOnProduct)(fakeRef, result).then(() => {
        assert("9a stamp called once", calls.length, 1);
        assert("9b stamp uses merge:true", calls[0].opts, { merge: true });
        assert("9c stamp payload keys", Object.keys(calls[0].payload).sort(), [
            "ai_blocker_count",
            "blocker_count",
        ...
        const required = (0, completionCompute_1.getRequiredFieldKeysPure)(fakeRegSnap);
        assert("10 getRequiredFieldKeysPure", required, [
            { field_key: "department", display_label: "Department" },
...
//# sourceMappingURL=completionCompute.test.js.map