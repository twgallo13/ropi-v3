...
// ── GET /conflicts ──
router.get("/conflicts", auth_1.requireAuth, (0, roles_1.requireRole)(resolverRoles), async (_req, res) => {
    try {
        const snap = await db()
            .collection("products")
...
router.post("/conflict/:mpn/resolve", auth_1.requireAuth, (0, roles_1.requireRole)(resolverRoles), async (req, res) => {
    try {
        ...
        if (action === "accept_map") {
            ...
            await productRef.collection("attribute_values").doc("scom").set({ value: mapPrice, ...provenance }, { merge: true });
            await productRef.collection("attribute_values").doc("scom_sale").set({ value: mapPrice, ...provenance }, { merge: true });
            await (0, pricingExportQueue_1.queueForPricingExport)(mpn, "map_change", userId, null);
...
// ── GET /removals ──
router.get("/removals", auth_1.requireAuth, (0, roles_1.requireRole)(resolverRoles), async (_req, res) => {
    try {
        ...
        const items = snap.docs
            ...
            return {
                ...
                brand: p.brand || "",
                map_price: p.map_price || 0,
                map_removal_proposed_at: p.map_removal_proposed_at?.toDate?.()?.toISOString() || null,
            ...
            .filter((i) => !i.map_removal_review_after ||
...
router.post("/removal/:mpn/resolve", auth_1.requireAuth, (0, roles_1.requireRole)(resolverRoles), async (req, res) => {
    try {
        const { mpn } = req.params;
        ...
        if (action === "approve_removal") {
            ...
            await (0, pricingExportQueue_1.queueForPricingExport)(mpn, "map_removal", userId, null);
            await db().collection("audit_log").add({
                product_mpn: mpn,
...
//# sourceMappingURL=mapReview.js.map