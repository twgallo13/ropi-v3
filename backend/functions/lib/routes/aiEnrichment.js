...
async function loadProductCtx(mpn) {
    const firestore = firebase_admin_1.default.firestore();
    const ref = firestore.collection("products").doc((0, mpnUtils_1.mpnToDocId)(mpn));
...
async function writeAttr(mpn, key, value, origin) {
    const firestore = firebase_admin_1.default.firestore();
    const ref = firestore
        .collection("products")
...
async function enrichName(mpn) {
    const ctx = await loadProductCtx(mpn);
    if (!ctx)
        return { error: "not_found" };
    // Nike/Jordan MPN normalization for the prompt
    const industryMpn = (0, ricsParser_1.getNikeIndustryMpn)(mpn, ctx.brand || "");
...
Respond with ONLY the product name, nothing else.`;
    ...
    const firestore = firebase_admin_1.default.firestore();
    const ref = firestore.collection("products").doc((0, mpnUtils_1.mpnToDocId)(mpn));
    await ref.set({ name: cleaned, name_source: "ai_generated", needs_ai_name: false }, { merge: true });
    const result = await writeAttr(mpn, "name", cleaned, "AI Name Enrichment");
    return { mpn, name: cleaned, ...result };
}
async function enrichColor(mpn) {
    const ctx = await loadProductCtx(mpn);
    if (!ctx)
        return { error: "not_found" };
    ...
    const result = await writeAttr(mpn, "descriptive_color", cleaned, "AI Color Enrichment");
    return { mpn, descriptive_color: cleaned, ...result };
}
router.post("/name/:mpn", async (req, res) => {
    try {
...
router.post("/color/:mpn", async (req, res) => {
    try {
