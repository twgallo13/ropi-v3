...
async function getSiteOwner(firestore, docId, data) {
...
router.post("/:mpn/complete", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (scom_sale > 0 && scom > 0 && scom_sale > scom) {
            discrepancyReasons.push(`Web sale price ($${scom_sale}) exceeds web regular price ($${scom})`);
        }
        if (discrepancyReasons.length > 0) {
            ...
            res.status(200).json({
                ...
                completed_by: req.user?.uid || "unknown",
                message: "Product completed but blocked by pricing discrepancy. Flag for re-import.",
            });
            return;
        }
...
router.post("/:mpn/attributes/:field_key", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        let mapAutoPopulate = { triggered: false };
        if (fieldKey === "map") {
            const mapValStr = finalValue === null || finalValue === undefined
                ? ""
                : String(finalValue).trim();
            const upper = mapValStr.toUpperCase();
            const isMapActive = mapValStr !== "" && upper !== "NO" && upper !== "DISALLOWED";
            if (isMapActive) {
