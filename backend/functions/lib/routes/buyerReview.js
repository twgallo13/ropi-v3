...
function buildPhase1Recommendation(product) {
    ...
    return {
        ...
        export_price: (0, pricingUtils_1.apply99Rounding)(newRicsOffer),
...
router.get("/", async (req, res) => {
    try {
        const { department, brand, site_owner, map_status, sort = "aging", limit: limitStr = "50", cursor, } = req.query;
...
router.get("/price-projection/:mpn", async (req, res) => {
    try {
        ...
        const cost = d.actual_cost && d.actual_cost > 0
            ? d.actual_cost
        ...
        const steps = markdowns.map((pct, i) => {
            ...
            const exportPrice = pct === 0 ? ricsOffer : (0, pricingUtils_1.apply99Rounding)(ricsOffer);
            const gmPct = ricsOffer > 0
...
//# sourceMappingURL=buyerReview.js.map